		...
		<div class="card">
			<div class="card-body" style="padding:10px 1.25rem 5px">
				...
				<script type="text/javascript">
					...
					function getSearch() {
						setTimeout(function() { self.location = "/settings/temp_pdf/?search="+jQuery("#search").val(); }, 50);
					}
				...
				<div class="row">																
					<div class="mb-1 col-sm-12 col-md-5 mt-1">
						<input type="text" onKeyDown="funcSearch(event);" placeholder="поиск по шаблону..." value="<?=$api->Strings->pr($_GET["search"])?>" id="search" class="form-control form-control-sm" />
						<a class="btn btn-link search_bt" onclick="getSearch()"><i class="fas fa-search"></i></a>
					</div>	
						<? if ($_GET["search"]) { ?>
					<div class="mb-1 col-sm-12 col-md-1 mt-1">		
						<a data-toggle="tooltip" title="" class="btn btn-link btn-danger btn-lg sbros" data-original-title="Сбросить фильтры" href="/settings/temp_pdf/">
			...
			<div class="card-body" style="position:relative">				
				...
				<div class="table-responsive">
					...
					if (isset($_GET["search"]) && $_GET["search"] != '')
						$sql_get .= " AND (INSTR(`name_temp`, '".$api->Strings->pr($_GET["search"])."') OR INSTR(`name`, '".$api->Strings->pr($_GET["search"])."') OR INSTR(`adres`, '".$api->Strings->pr($_GET["search"])."') OR INSTR(`number`, '".$api->Strings->pr($_GET["search"])."') OR INSTR(`director`, '".$api->Strings->pr($_GET["search"])."'))";
		
					$order_name = '';
					$by_name = '';
					$order_by = "`name_temp` ASC, `id` DESC";
					if (
						(isset($_GET["order"]) && ($_GET["order"] == 'name_temp' || $_GET["order"] == 'name' || $_GET["order"] == 'director') ) &&
					...
					$i=1;
					$per_page = 1000;
					$sql_ = "FROM `i_temp_pdf` WHERE `name_temp` IS NOT NULL".$sql_get;
					...
					if (mysql_num_rows($s) > 0)
					{
						...
						<table id="basic-datatables" class="display table table-striped table-hover dataTable">
							<thead>
								<tr>									
									<th class="sorting<?=($order_name == 'name_temp' ? '_'.$by_name : '')?>" onclick="location.href='?order=name_temp&by=<?=($by_name == 'asc' && $order_name == 'name_temp' ? 'desc' : 'asc').$filter_gets?>'" nowrap style="padding-right:30px !important">Шаблон</th>
									<th class="sorting<?=($order_name == 'name' ? '_'.$by_name : '')?>" onclick="location.href='?order=name&by=<?=($by_name == 'asc' && $order_name == 'name' ? 'desc' : 'asc').$filter_gets?>'" nowrap style="padding-right:30px !important">Наименовании компании</th>
									<th>Адрес</th>
									<th>Номер / дата</th>
									<th class="sorting<?=($order_name == 'director' ? '_'.$by_name : '')?>" onclick="location.href='?order=director&by=<?=($by_name == 'asc' && $order_name == 'director' ? 'desc' : 'asc').$filter_gets?>'" style="padding-right:30px !important">Директор</th>									
								</tr>
							</thead>
						<?
						...
						{							
							...
							<tr role="row" class="<?=(($i%2)==1 ? 'odd' : 'even')?>">
								<td<?=$link?> nowrap="nowrap"><?=stripslashes($r["name_temp"])?></td>								
								<td<?=$link?> nowrap="nowrap"><?=$api->Strings->pr_plus(htmlspecialchars_decode(stripslashes($r["name"])))?></td>
								<td<?=$link?> nowrap="nowrap"><?=$api->Strings->pr_plus(htmlspecialchars_decode(stripslashes($r["adres"])))?></td>
								<td<?=$link?> nowrap="nowrap"><?=stripslashes($r["number"])?> / <?=stripslashes($r["date_do"])?></td>
								<td<?=$link?> nowrap="nowrap"><?=stripslashes($r["director"])?></td>										
							</tr>
							<?
							$i++;
						}
