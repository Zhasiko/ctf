	...
	)
	{
		...
		{			
			...
			if ($insert)					
				...
				<script type="text/javascript">
					jQuery("#protocol").html("<span style=\"color:#53b374\">Вы успешно добавили телефон</span>");
					setTimeout(function() { self.location = "/settings/phones/"; }, 2000);
				</script>';					
		}
		...
		)
		{
			...
			if ($update)						
				echo '
				<script type="text/javascript">
					jQuery("#protocol").html("<span style=\"color:#53b374\">Вы успешно сохранили телефон</span>");
					setTimeout(function() { self.location = "/settings/phones/"; }, 2000);
				</script>';							
		}
		...
		{			
			...
			$s=mysql_query("SELECT `id` FROM `i_dir_phone` WHERE `id`='".$id."'");
			if (mysql_num_rows($s) == 1)
			{
				$r=mysql_fetch_array($s);
									
				$sql_delete = "DELETE FROM `i_dir_phone` WHERE `id`='".$r["id"]."'";
				$delete = mysql_query($sql_delete);
					
				echo '
				<script type="text/javascript">
					jQuery("#protocol").html("<span style=\"color:#53b374\">Вы успешно удалили телефон</span>");
					setTimeout(function() { self.location = "/settings/phones/"; }, 50);
				</script>';
