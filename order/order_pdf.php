...
/*
$user_info = $api->Strings->change_s(trim(stripslashes($r["user_name"]).'  '.stripslashes($r["user_uyr_adres"]).'  '.stripslashes($r["user_fac_adres"]).($r["user_phone"]!='' ? '  тел: '.stripslashes($r["user_phone"]) : '').($r["user_fax"]!='' ? '  факс: '.stripslashes($r["user_fax"]) : '').($r["user_mail"]!='' ? '  email: '.stripslashes($r["user_mail"]) : '')));

//$x_name = str_split($user_info);
$x_name = Array();
$len = mb_strlen($user_info, 'UTF-8');
for ($i = 0; $i < $len; $i++) {
	$x_name[] = mb_substr($user_info, $i, 1, 'UTF-8');
}
...
<div style="position:relative; width:590px; height:1080px; font-family:Microsoft Sans Serif; margin:0 0 0 110px; padding-top:170px">
	...
	<div style="font-size:12px; line-height::22px; text-align:center; margin:5px 0">ОБЩИЕ ХАРАКТИРИСТИКИ ТРАНСПОРТНОГО СРЕДСТВА</div>
	<table style="width:100%;" border="1" cellpadding="0" cellspacing="0.5">
	';
...
<div style="position:relative; width:590px; height:1080px; font-family:Microsoft Sans Serif; margin:0 0 0 110px; padding-top:90px">
	...
	foreach($fields_3 as $k=>$v)
	{
		...
		*/
		...
		<tr>
			<td style="'.$style_td_left.'" align="left" valign="middle">'.$name_ru[$v].'</td>
			<td style="'.$style_td_right.'" align="left" valign="middle">'.($r[$v]!='' ? $api->Strings->pr_plus(htmlspecialchars_decode(stripslashes($r[$v]))).' ' : '').($v == 'dop_obor' && intval($r["uvesos"]) == 1 ? 'Оснащен устройством вызова экстренных оперативных служб
"УВС-М"., ICCID' : '').'</td>
		</tr>
		';
	}	
...
$strPdf .= '
	...
	<div style="font-size:12px; line-height:12px; padding:5px 0 20px;">соответсвует требованиям технического регламента Таможенного соэза "О безопасности колесных транспортных средств".</div>
